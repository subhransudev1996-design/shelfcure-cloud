    ...
    <div className="grid min-h-screen lg:grid-cols-5">
      ...
      <aside className="relative hidden overflow-hidden bg-zinc-950 lg:col-span-2 lg:flex lg:flex-col lg:justify-between lg:p-12">
        <div
          ...
          style={{
            background:
              'radial-gradient(circle at 20% 10%, rgba(99,102,241,0.4), transparent 50%), radial-gradient(circle at 80% 90%, rgba(67,56,202,0.3), transparent 45%), linear-gradient(135deg, #0c0a09 0%, #18181b 100%)',
          }}
        />
        <div
        ...
        <div className="relative space-y-6">
          ...
          <div className="flex items-center gap-3 text-sm text-indigo-200/90">
            <span className="inline-flex h-1.5 w-1.5 rounded-full bg-indigo-400 shadow-[0_0_12px_2px_rgba(129,140,248,0.7)]" />
            Internal tool · ShelfCure staff only
          </div>
        </div>

        <div className="relative grid grid-cols-3 gap-4 text-xs text-zinc-400">
          <Stat label="Orgs visible" value="All" />
          <Stat label="License control" value="Full" />
          <Stat label="Access tier" value="Platform" />
        </div>
      </aside>
